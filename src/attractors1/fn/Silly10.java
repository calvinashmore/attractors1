 ...
 * @author ashmore
 */
public class Silly10 extends AbstractFn {

  public Silly10(ArrayParams parameters) {
    super(parameters);
  }
  ...
  public Point3d apply(Point3d v) {
    ...
    // another sawtooth
    // scales to 0-1
    double xf = (v.getX() % (5*p[0])) / (5*p[0]);
    double yf = (v.getY() % (5*p[1])) / (5*p[1]);
    double zf = (v.getZ() % (5*p[2])) / (5*p[2]);
